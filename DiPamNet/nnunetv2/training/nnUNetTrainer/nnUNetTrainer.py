...
class nnUNetTrainer(object):
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        if isinstance(filename_or_checkpoint, str):
            ...
            #filename_or_checkpoint_path = os.path.dirname(os.path.abspath(filename_or_checkpoint))
            checkpoint = torch.load(filename_or_checkpoint, map_location=self.device, weights_only=False)
            #checkpoint = torch.load(os.path.join(filename_or_checkpoint_path,'checkpoint_best.pth'), map_location=self.device, weights_only=False)

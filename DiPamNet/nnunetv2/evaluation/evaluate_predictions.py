...
def save_summary_json(results: dict, output_file: str):
    ...
    results_converted['mean'] = {
        label_or_region_to_key(k): {
            'Dice': results['mean'][k]['Dice'],
            'IoU': results['mean'][k]['IoU'],
            'HD95': results['mean'][k]['HD95']  # Add HD95
        } for k in results['mean']
    }
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    spacing = seg_ref_dict.get('spacing', [1.0, 1.0, 1.0])  # may be 2 or 3 elems depending on metadata
    ...
    for r in labels_or_regions:
        ...
        # ---- FIX: make spacing_mm compatible with mask dimensionality ----
        spacing_mm = tuple(spacing) if spacing is not None else None
        if spacing_mm is not None:
            if mask_ref.ndim == 2 and len(spacing_mm) == 3:
                spacing_mm = spacing_mm[1:]  # drop z
            elif mask_ref.ndim == 3 and len(spacing_mm) == 2:
                spacing_mm = (1.0, *spacing_mm)

        # Compute HD95
        if np.any(mask_ref) and np.any(mask_pred):
            surface_distances = compute_surface_distances(
                mask_ref, mask_pred, spacing_mm=spacing_mm
            )
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    for r in regions_or_labels:
        hd95_values = [case['metrics'][r]['HD95'] for case in results if not np.isnan(case['metrics'][r]['HD95'])]
        means[r]['HD95'] = np.mean(hd95_values) if hd95_values else np.nan
    
    # Compute foreground HD95 mean
    foreground_hd95 = []
    for k in means:
        if k == 0 or k == '0':
            continue
        if not np.isnan(means[k]['HD95']):
            foreground_hd95.append(means[k]['HD95'])
    foreground_mean['HD95'] = np.mean(foreground_hd95) if foreground_hd95 else np.nan

...
def encode(p, n, coords):
    hc = HilbertCurve(p, n)
    return hc.distance_from_coordinates(coords)

def decode(p, n, distance):
    hc = HilbertCurve(p, n)
    return hc.coordinates_from_distance(distance)
...
class HSCANS(nn.Module):
    ...
    def __call__(self, img):
        ...
        return img_encode
    def encode(self, img):
        B, C, L = img.shape  # Get dynamic spatial dimensions
        device = img.device
        # index_flat_inv = self.index_flat_inv.expand(B, C, L).to(device)
        # img_encode = torch.zeros_like(img).scatter_(2, index_flat_inv, img)
        index_flat_inv = torch.arange(L, device=device).view(1, 1, L)
        img_encode = torch.zeros_like(img).scatter_(
            2, 
            index_flat_inv.expand(B, C, L), 
            img
        )
        return img_encode
    ...
    def decode(self, img):
        B, C, L = img.shape
        device = img.device
        # index_flat = self.index_flat.expand(B, C, L).to(img.device)
        # img_decode = torch.zeros_like(img).scatter_(2, index_flat, img)
        index_flat = torch.arange(L, device=device).view(1, 1, L)
        img_decode = torch.zeros_like(img).scatter_(
            2, 
            index_flat.expand(B, C, L), 
            img
        )
        return img_decode
